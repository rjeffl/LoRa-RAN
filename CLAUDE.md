...
|---|---|---|
| `LRAN-Protocol-Specification` | `docs/shared/` | **Every byte on the wire and every MQTT topic.** No other document may redefine a frame layout, enum value, schema ID or topic. **Currently v0.16, `ver = 2`** |
| `LRAN-Decision-Register` | `docs/shared/` | **D1–D72** and measurement backlog **M1–M26**. The only place a decision's status is recorded |
