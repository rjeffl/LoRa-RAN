...
|---|---|---|
| `LRAN-Protocol-Specification` | `docs/shared/` | **Every byte on the wire and every MQTT topic.** No other document may redefine a frame layout, enum value, schema ID or topic. **Currently v0.13, `ver = 2`** |
...
- **One task group per session.** Start from the *Start here* section of the node's
  ...
  wrap up before it passes about 40 %.
