...
- **Its files differ from the bridge's in three ways.** It writes no `FRAME` lines, so
  ...
  lines counts LRAN-PHY frames it heard, and it keeps sampling through them. `main.cpp` says
  why.
...
- **The XIAO has not run a capture yet.** Its native USB drops output written before the
