...
- A duplicate index within a live set overwrites the stored fragment and is counted
  ...
  vectors generated from a conforming sender. Tracked as **W13**.
...
| W4 | ~~Test vectors~~ | — | **Closed.** `/tools/vectors/` exists: an independent Python generator, a self-check and **71** vectors, cross-checked against a C++ suite on host and on target with zero divergence. The generator was written from this document alone, with the codec off limits, and the four disagreements it produced are the substance of v0.5. Provenance is recorded per vector — 65 derived, 6 adjudicated — so a reader can tell which agreements are evidence and which are bookkeeping. The v0.5 regeneration changed **no existing frame byte**, independently confirming that v0.5 altered no header field, authentication scope or schema layout. §14.1 is enforced by the generator and the checker independently. §13.2's standing requirement to regenerate on every protocol change is unaffected and applies to v0.6 |
...
| W13 | **No vector reaches §11.2's dead-space clause** | §11.2 | A duplicate fragment of differing length exhausting staging is a **non-conforming-sender** path, and W4's generator emits conforming senders by construction, so the case cannot be expressed in the fragmentation vector shape. Covered by unit test today. Either accept unit-test coverage as sufficient and say so here, or add a raw-frames vector form that can express a malformed set. Not urgent; it is the only normative clause in §11 with no vector behind it |
...
- **v0.6** — Decision capture, plus three findings from the v0.5 implementation.
  ...
  **W13** added: §11.2's dead-space clause is a non-conforming-sender path that W4's
  ...
  after alignment — and W4's closure note records 71 vectors, per-vector provenance
  (65 derived, 6 adjudicated) and that the v0.5 regeneration changed no existing frame
  byte. **W5 closes** (§18.1): a single fixed channel, no hopping,
