...
> Retaining the completed key costs ten bytes per slot and removes all three.
>
...
> **508 B** on ESP32-S3, so a bridge holding one per provisioned node costs about
> **2.5 KB for five nodes** — under 1 % of the part's 320 KB. There is no reason to
> share a set across peers, and a bridge that does will say so through
...
| Counter | Raised at | In `rx_dropped` |
|---|---|:---:|
| `rx_crc_err` | §14 stage 1 | yes |
| `rx_runt` | §14 stage 2 | yes |
| `rx_oversize` | §14 stage 2a | yes |
| `rx_bad_crc` | §14 stage 3 | yes |
| `rx_bad_ver` | §14 stage 4 | yes |
| `rx_not_addressed` | §14 stage 5 | yes |
| `rx_unknown_hdr_ext` | §14 stage 5a | yes |
| `rx_bad_frag` | §14 stage 5b | yes |
| `rx_unknown_type` | §14 stage 6 | yes |
| `rx_unknown_schema` | §14 stage 7 | yes |
| `rx_bad_length` | §14 stage 8 | yes |
| `rx_not_fragmentable` | §14 stage 8a | yes |
| `rx_rejected_ctx` | §14 stage 9 / §9.4 step 2 | yes |
| `rx_rejected_mac` | §14 stage 9 / §9.4 step 3 | yes |
| `rx_reassembly_timeout` | §11.2 — incomplete set expired | yes |
| `rx_fragment_overflow` | §11.2 — index ≥ total, cap exceeded, staging exhausted | yes |
| `rx_reassembly_abandoned` | §11.3 — live set displaced | yes |
| `rx_rejected_seq` | §14 stage 11 / §9.4 step 5 | yes |
| `rx_frag_duplicate` | §11.2 — duplicate index within a live set | **no** |
| `rx_frag_late` | §11.2 — fragment matching the last completed set | **no** |
| `rx_dup_command` | §14 stage 11 / §9.4 step 4 — dedup cache hit | **no** |
...
| W4 | ~~Test vectors~~ | — | **Closed.** `/tools/vectors/` exists: an independent Python generator, a self-check and 69 vectors in four groups, cross-checked against a C++ suite on host and on target with zero divergence. The generator was written from this document alone, with the codec off limits, and the four disagreements it produced are the substance of v0.5. §13.2's standing requirement to regenerate on every protocol change is unaffected and applies to v0.5 |
