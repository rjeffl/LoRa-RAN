...
| **BF-24** | **Publication policy** — `publish.cpp`, §6.3's whole table. **Built and host-tested 2026-09-23**, Impl Plan §6.3.1: five documents from schema `0x10` and one from `0xF0`, a stale block published as unavailable, cell voltages behind a deadband, a heartbeat, the synthetic mark, SNTP for `last_traversal`, and GateLink's discovery entities. Three bridge rows came with it, chosen with the operator: `republish_interval_s`, `bms_stale_s` and `cell_mv_deadband`. **Shown in HA on 2026-09-24** with BF-27's dummy publish (Impl Plan §6.6.2): synthetic marked, stale blocks unavailable through HA restarts. Events are BF-25's | **Opus** | **R-5.2b is the requirement most easily lost in implementation**, because republishing the cached value is the path of least resistance and produces a dashboard that looks healthy. A dead VE.Direct link showing plausible unchanged numbers indefinitely is worse than an obviously unavailable entity |
...
| **BF-37** | **Events at QoS 1** — D5's designated fallback, espMqttClient, behind `MqttTransport` (§4.3, Impl Plan §6.3.2). **Added 2026-09-25. Built and shown at the broker 2026-09-25**, Impl Plan §4.3.3. PubSubClient 2.8 publishes at QoS 0 whatever the message asks, and spec §16.3 requires QoS 1 for events. `make_publish()` already refuses an event at QoS 0, so the tag is right and the transport drops it. No milestone gates it; build it before GateLink's events drive email and SMS | **Opus** | A transport swap under every publish the bridge makes. The failure it closes is silent: an event lost when a connection drops mid-publish looks like an event that never happened. Its reconnect and LWT behaviour must match BF-12's, which V-B4 proved |
| **BF-38** | **The publish queue's per-class policy** (Impl Plan §5.2.1). **Added 2026-09-25. Built 2026-09-25** as a separate event queue, and shown across a broker restart, Impl Plan §4.3.3. A full queue refuses the newest message, which suits neither class: state is idempotent and could drop its oldest, and an event has no replacement. While the broker is down, state fills the 32 slots and a new event is lost. §5.2.1 gave this refinement to BF-24 and BF-25, and neither built it. No milestone gates it | **Opus** | The queue is what rides out a broker outage, and the event lost is the one an automation was waiting for. B7's broker-restart criterion is where a wrong answer shows, and only if an event happens to fall in the outage |
...
| **BF-28** | HEX wrap/unwrap, request and response plumbing, retained audit trail (§6.4). **Built and host-tested 2026-09-25**, Impl Plan §6.4.1: `hex_proxy.{h,cpp}`. A write shares the command `seq` space and is never retried; a read is retried under the same `seq`. Not yet on air | **Sonnet** | Transport and logging. The node is transport only; the shape is written out in §6.4's flow |
| **BF-29** | **Write authorization** — arm, auto-expiry, refusal while disarmed, the three gates (§3.5b, **BS-2**). **Built and host-tested 2026-09-25**, Impl Plan §6.4.1: `WriteArm`, and `test_hex_proxy` tests each gate on its own. A retained `write_enable/set` is ignored and cleared, chosen with the operator. **V-B6 has not run on the bench** | **Opus** | *"The failure mode is battery damage, and it is invisible until it is not."* Three gates are only three gates if each is independently enforced and independently tested (**V-B6**) |
...
- **v0.51** — **BF-36 and BF-28 to BF-30 are built and host-tested.** Their rows record it;
  none has been on air. The Impl Plan citation moves to v0.64, whose new §6.4.1 is the
  record.
...
- **v0.49** — **Spec v0.15's code ran on air, without task rows.** No task row changes.
  The Impl Plan citation moves to v0.62, whose §6.7.2a and §10.2 record the bench run's
  two bridge fixes and the simnode's `CONFIG_CHANGE`; nothing here inherits them.
...
- **v0.47** — **Protocol specification v0.14 → v0.15.** No task row changes. The code
  ...
  Bridge PRD, Impl Plan and Library Plan move to v0.16, v0.60 and v0.20.

- **v0.46** — **Three tasks added from the pre-GateLink survey.** **BF-36**, a simulated
  MPPT in the simnode, which B5 needs before it can run on the bench. **BF-37**, events at
  QoS 1, and **BF-38**, the publish queue's per-class policy, both open in the bridge
  handoff since BF-25 with no task row. BF-13's row now records that V-B9 passed, and
  BF-14's that the page was seen, both at B2's bench session on 2026-09-13.
...
- **v0.39** — **BF-24 is built and host-tested**; its row says what is done and what is
  not. The Impl Plan citation moves from v0.49 to v0.50, which adds §6.3.1.

- **v0.38** — **BF-26 is built and confirmed on air**; its row says how. The Impl Plan
  citation moves from v0.48 to v0.49, which adds §4.2a.1.
