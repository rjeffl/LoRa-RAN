...
| [`LRAN-Protocol-Specification`](./shared/LRAN-Protocol-Specification.md) | All LoRa frame and MQTT protocol definitions. **Referenced by every node document** | **v0.16** (`ver = 2`) |
| [`LRAN-Decision-Register`](./shared/LRAN-Decision-Register.md) | **D1–D72** and the measurement backlog **M1–M26**. Single source of truth for decision status | v0.23 |
| [`LRAN-Protocol-Library-Implementation-Plan`](./shared/LRAN-Protocol-Library-Implementation-Plan.md) | `/lib/lran-protocol/` API, tests and milestones. **P1–P8 complete** | v0.22 |
...
| [`LRAN-Bridge_Node-PRD`](./bridge/LRAN-Bridge_Node-PRD.md) | Bridge Node goals and requirements | v0.17 |
| [`LRAN-Bridge_Node-Implementation-Plan`](./bridge/LRAN-Bridge_Node-Implementation-Plan.md) | Bridge Node BOM, firmware architecture, milestones; also owns `lran-simnode` (§10) | v0.67 |
| [`LRAN-Bridge-Firmware-Tasks`](./bridge/LRAN-Bridge-Firmware-Tasks.md) | Bridge and simnode task breakdown under B0–B7, work order, and model suitability per task. **Owns no requirement** | v0.54 |
...
| [`LRAN-GateLink_Node-PRD`](./gatelink/LRAN-GateLink_Node-PRD.md) | GateLink goals and requirements | v0.12 |
| [`LRAN-GateLink_Node-Implementation-Plan`](./gatelink/LRAN-GateLink_Node-Implementation-Plan.md) | GateLink BOM, interconnect, firmware architecture, milestones, integration observations | v0.16 |
...
| [`LRAN-WellLink_Node-PRD`](./welllink/LRAN-WellLink_Node-PRD.md) | WellLink — placeholder, to be developed | v0.12 |
...
- **v0.27** — **Protocol specification v0.15 → v0.16.** No architecture changes. A
  request that may have executed before a node reset is no longer retried, and spec §10.7
  gathers the reset cases. §12's version column follows the sweep: the Decision Register to
  v0.23 and **D1–D72**, and seven node and library documents to their current revisions,
  three of which the column had fallen behind.
...
- **v0.22** — **Protocol specification v0.13 → v0.14.** No architecture changes. D59
  ...
  the bridge's topic alone, and no node commits until the bridge has heard every node on
  the new settings. §12's version column had fallen behind again in six rows, and now
  matches each document's header.

- **v0.21** — **Protocol specification v0.12 → v0.13.** No architecture changes. The
  specification defines the configuration payloads (**D43–D57**), allows runtime PHY
  changes under commit-and-revert (**D56**), and adds a context roll after a bridge
  restart (**D58**). §2.3's non-goal of persisting a sequence counter across reboots still
  holds: D58 considered persisting the bridge's command `seq` and rejected it. §12's
  version column had fallen behind again in nine rows, and now matches each document's
  header.
...
- **v0.18** — **Protocol specification v0.11 → v0.12, and §12's version column corrected.**
  ...
  discard stage and counter (§14 stage 9a, `rx_unknown_src`), pre-authentication counters
  belonging to the receiver rather than to a node (§14.1), an `ERROR` policy for
  unauthenticated senders (§14.2), `CONFIG` and `CONFIG_ACK` single-frame (§11.4), and
  normative payloads for the two MQTT topics that ship (§16.2.1). **§12's version column
  was wrong in eight rows**, the Bridge Implementation Plan by fourteen revisions, so the
  rows are corrected and the note beneath now says what is enforced and what is not.
  Decision Register **D35–D42**, **M24**.
