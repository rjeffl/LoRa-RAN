...
Overhead is **14 bytes** unauthenticated, **22 bytes** authenticated.
...
| `0xF0`–`0xFE` | **Bench and simulator nodes** — `lran-simnode` instances |
| `0xFF` | Broadcast |
...
- The payload shapes genuinely differ. `EVENT` is a small record of *something that
  ...
  serve both means a 76-byte frame for a fire alarm.
...
Used for RF loopback and link testing. A responder swaps `src`/`dst`, preserves `seq`
and the echo bytes, and retransmits.
...
| `0xFE` | **Simnode synthetic status** — bench only | 76 B (mirrors `0x10`) |
...
### 7.2 Schema `0x10` — GateLink status v1 (76 bytes)

#### 7.2.1 Gate block — offsets 0–7
...
| 6 | `uint16` | `last_traversal_age_s` | Seconds since the last classified traversal. `UINT16_MAX` = none since boot; saturates rather than wraps |
...
#### 7.2.2 MPPT block — offsets 8–33

| Off | Type | Field | Unit | VE.Direct |
|---:|---|---|---|---|
| 8 | `uint16` | `batt_mv` | mV | `V` |
| 10 | `int16` | `batt_ma` | mA | `I` — negative = discharge |
| 12 | `uint16` | `pv_cv` | **10 mV** | `VPV`. 10 mV units because the 75/15 accepts up to 75 V and mV would overflow `uint16` at 65.5 V |
| 14 | `uint16` | `pv_w` | W | `PPV` |
| 16 | `int16` | `load_ma` | mA | `IL`. `INT16_MIN` = not available |
| 18 | `uint16` | `yield_today` | 10 Wh | `H20` |
| 20 | `uint16` | `yield_yest` | 10 Wh | `H22` |
| 22 | `uint16` | `pmax_today` | W | `H21` |
| 24 | `uint32` | `yield_total` | 10 Wh | `H19`. `uint32` — `uint16` overflows at 655 kWh, reachable in a few years |
| 28 | `uint8` | `charge_state` | — | `CS`, passed through |
| 29 | `uint8` | `mppt_err` | — | `ERR`, passed through. Non-zero triggers a push |
| 30 | `uint8` | `mppt_tracker` | — | `MPPT`, passed through |
| 31 | `uint8` | `mppt_flags` | — | §7.2.6 |
| 32 | `int16` | `mppt_temp_c10` | 0.1 °C | Second of three thermal reference points. `INT16_MIN` = not available |
...
|---:|---|---|---|---|
| 34 | `uint8` | `bms_soc` | % | `0xFF` = not available |
| 35 | `uint8` | `bms_flags` | — | §7.2.7 |
| 36 | `uint16` | `pack_mv` | mV | |
| 38 | `int16` | `pack_ma` | mA | Sign convention pending — see §18, W6 |
| 40 | `uint8` | `cell_count` | — | Cells actually reported. `>4` requires a new schema ID |
| 41 | `uint8` | `bms_rssi_neg` | −dBm | Magnitude of the BLE RSSI, e.g. `80` means −80 dBm. `0` = no link. Ongoing evidence for GateLink **D28** |
| 42 | `uint16` | `cell_mv[0]` | mV | |
| 44 | `uint16` | `cell_mv[1]` | mV | |
| 46 | `uint16` | `cell_mv[2]` | mV | |
| 48 | `uint16` | `cell_mv[3]` | mV | |
| 50 | `int8` | `cell_temp_c[0]` | °C | |
| 51 | `int8` | `cell_temp_c[1]` | °C | |
| 52 | `int8` | `cell_temp_c[2]` | °C | |
| 53 | `int8` | `cell_temp_c[3]` | °C | Third thermal reference point (with `mppt_temp_c10` and `enclosure_temp_c10`) |
| 54 | `uint16` | `bms_cycles` | — | |
| 56 | `uint16` | `bms_capacity_dah` | 0.1 Ah | Reported pack capacity |
| 58 | `uint16` | `bms_alarms` | — | Protection/alarm bitfield, passed through from the BMS unmodified |
| 60 | `uint16` | `bms_age_s` | s | Seconds since the last successful BMS read. Saturates at `UINT16_MAX` |
...
#### 7.2.4 Node block — offsets 62–75

| Off | Type | Field | Unit | Notes |
|---:|---|---|---|---|
| 62 | `uint32` | `uptime_s` | s | |
| 66 | `uint16` | `boot_count` | — | From nonvolatile storage; `0` if unavailable |
| 68 | `uint16` | `node_mv` | mV | INA226 supply voltage — the node measures its own rail |
| 70 | `int16` | `node_ma` | mA | INA226 supply current |
| 72 | `int16` | `enclosure_temp_c10` | 0.1 °C | LM75. First of three thermal reference points |
| 74 | `uint8` | `node_flags` | — | §7.2.8 |
| 75 | `uint8` | `status_reason` | — | §8.7 — why this frame was sent |
...
mac = HMAC-SHA256( node_key, ver || type || src || dst || seq || ctx_id
                             || frag || schema || payload )[0..7]
```

That is **the entire 12-byte header followed by the entire payload**, serialized
...
- A fragment index ≥ the declared total, or a set exceeding the 200-byte payload cap on
  reassembly, is discarded with `ERROR(FRAGMENT_OVERFLOW)`.
...
No schema currently defined requires fragmentation — the largest is 76 bytes.
...
| Reserved bit or field assigned a meaning | No bump — §4.3 requires receivers to ignore them |
...
|---|---:|---:|---:|---:|
| `POLL` | 15 | 41 ms | 82 ms | 144 ms |
| `COMMAND` | 26 | 56 ms | 103 ms | 185 ms |
| `COMMAND_ACK` | 20 | 46 ms | 92 ms | 165 ms |
| `EVENT` (`0x11`) | 30 | 62 ms | 113 ms | 206 ms |
| Node health (`0xF0`) | 34 | 67 ms | 123 ms | 226 ms |
| **`STATUS` (`0x10`)** | **90** | **149 ms** | **267 ms** | **493 ms** |
...
A `STATUS` frame at SF9 costs ~493 ms of TX. At ~118 mA at +22 dBm on the 3.3 V rail
...
**Design implication: do not optimize these schemas for byte count.** The 76-byte
...
| Element | Values |
|---|---|
| `<node>` | `bridge`, `gatelink`, `welllink`, `simnode<N>` |
...
| `src`/`dst` | `0xF0`–`0xFE` | Bench and simulator nodes |
...
`simnode` targets register in the `0xF0`–`0xFE` range, answer polls with schema `0xFE`
...
| W7 | Airtime table regeneration | §15.1 | Recompute once **D1** fixes SF/BW/CR |
...
```
header (12 B, all frames)
  [ver][type][src][dst][seq:2][ctx_id:4][frag][schema]

COMMAND      (26 B)  header + [cmd][arg][arg2:2]                 + mac:8 + crc:2
COMMAND_ACK  (20 B)  header + [ack_seq:2][result][detail][rsv:2]          + crc:2
POLL         (15 B)  header + [poll_flags]                                + crc:2
STATUS 0x10  (90 B)  header + [ 76-byte payload, §7.2 ]                   + crc:2
EVENT  0x11  (30 B)  header + [ 16-byte payload, §7.3 ]                   + crc:2
CONFIG 0x12  (var )  header + [op][count][ entries... ]          + mac:8 + crc:2
CONFIG_ACK   (var )  header + [op][persist][count][ results... ]          + crc:2
ERROR        (18 B)  header + [err_code][detail][ref_seq:2]               + crc:2
PING       (15+N B)  header + [n][data:N]                                 + crc:2
HEX_REQ    (16+N B)  header + [flags][n][hex:N]              (+ mac:8)    + crc:2
HEX_RSP    (17+N B)  header + [status][n][hex:N]                          + crc:2
HEALTH 0xF0  (34 B)  header + [ 20-byte payload, §7.5 ]                   + crc:2
```
