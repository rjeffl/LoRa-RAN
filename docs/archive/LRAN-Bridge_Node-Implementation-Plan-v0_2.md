...
**Firmware targets:** `lran-bridge`, `lran-simnode` (§10)
...
| *1* | *Heltec WiFi LoRa 32 V3* | *Third board — contention testing and cold spare. See §2.1* |
...
**Recommendation: buy the third board.** At ~$20 it is the cheapest line item in the
project, and it buys three separate things — real contention testing, a cold spare for
the bridge (the single node whose failure takes the entire property's telemetry with it),
and the freedom to leave a simnode permanently deployed without losing the range-test
rig. See §10.7 for why the bench peer should stay in service permanently rather than being
reclaimed.

**Do not buy a fourth for GateLink development.** GateLink's radio is a `win-sx1262`
module on a StamPLC carrier, with a different pin map, TCXO voltage and RF-switch mode
(§3.1). A Heltec cannot stand in for that bring-up, and a green bench run against a Heltec
simnode says **nothing** about whether the carrier's radio configuration is correct. The
simnode validates the *protocol*; the carrier validates the *radio*. Keeping those two
claims separate is what prevents an afternoon lost to "but it worked on the bench."
...
/firmware/
  ...
  simnode/              <-- §10
/lib/
  lran-protocol/        frame codec, shared by every target and by /tools/
  lran-config/          parameter registry: ids, types, ranges, defaults
  lran-sim/             synthetic payload generators + malformed-frame
                        primitives. Shared by simnode AND /tools/ so the
                        host-side test-vector generator and the on-air fault
                        injector produce byte-identical frames
  vedirect/
/tools/
  simctl/               host script driving a simnode over USB serial
  vectors/              test-vector generation and checking (W4)
/docs/
...
|---|---|---|---|
| **B1** | **RF link characterization** | Two Heltec boards, `ROLE_RANGE` | RSSI and SNR measured at ~500 ft **on both the gate bearing and the well bearing**, across candidate SF/BW/CR settings. **D1 resolved** with a stated link margin. Bridge antenna type and position chosen and recorded. Airtime table regenerated (**M19**). FCC operating mode question (**W5**) settled before a TX power is fixed. **PHY-CRC discard counters observed at the far edge of the link** (§10.5) |
...
**B0 sits before B3 and can be built in parallel with B2.** It is deliberately separated
...
**Hardware:** Heltec WiFi LoRa 32 V3, unmodified. Identical to the bridge board.
...
- **v0.2** — Rebased on [`LRAN-Protocol-Specification`](./LRAN-Protocol-Specification.md)
  **v0.3** and expanded to define the bench fleet as buildable firmware rather than a
  ...
  `win-sx1262` carrier bring-up. **New §2.2**, bench RF hygiene — never key up without an
