...
control. If confirmed, it costs **one additional GPIO in the StamPLC pin budget** and adds
a sleep-current item at the gate. Seeed has not published a schematic for the module
itself — only a wiring diagram for the XIAO carrier — so **this cannot be settled from
documentation and must be measured on the board.** That is an argument for the evaluation
purchase independent of its simnode role.
...
"with XIAO ESP32S3" kit. **Record the exact part number of the board that arrives in
`/docs/simnode/engineering-log.md`, and order that same variant for the StamPLC carrier.**
A pin-configuration difference discovered after the carrier is built is a re-spin.
...
SPI pins, TCXO voltage, DIO2-as-RF-switch, and (pending §2.3.1) an optional RXEN pin.
