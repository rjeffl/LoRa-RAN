...
DEV_MAC = "C0:D6:58:3C:49:A1"          # Linux only; macOS hides MACs
...
async def find_device(timeout=15.0):
    ...
    if dev is None and sys.platform.startswith("linux"):
        dev = await BleakScanner.find_device_by_address(DEV_MAC, timeout=8.0)
    return dev


async def attempt(dev, reg, response, notify_both, delay, repeat, dump, quiet=False):
