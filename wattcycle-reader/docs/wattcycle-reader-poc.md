...
You have nRF Connect and the vendor app on the phone, which means the single biggest risk in §15 — "is this actually a JBD-compatible BMS?" — can be settled in five minutes with no code at all.

**Procedure:**

1. Force-quit the vendor app. Only one central at a time.
2. nRF Connect → scan → connect to `XDZN-001-49A1` (`C0:D6:58:3C:A1:49`).
3. Expand service `0xFFF0`. **Note the properties listed under `FFF2`** — specifically whether it says WRITE, WRITE NO RESPONSE, or both. Write this down; the firmware has to match.
4. On `FFF1`, tap the "enable notifications" icon (the triple down-arrow).
5. On `FFF2`, write the byte value `DDA50300FFFD77`.
6. Watch `FFF1`.

**Interpreting what comes back:**

- Notification beginning `DD 03 00 ...` and ending `77` → confirmed JBD protocol, decode table in §5 applies, proceed with confidence.
- Notification arrives in **two or three chunks** → expected, and it confirms the reassembly requirement is real rather than theoretical.
- Status byte non-zero (`DD 03 80 ...`) → command rejected; the register or framing differs.
- Nothing at all → either notifications didn't enable, the write type is wrong, or this variant needs an unlock/auth step first.

Then open the vendor app and record SOC, pack voltage, current and temperature at that moment. That's your ground truth for M5 — capture it as a note in the repo alongside the raw hex, because you will want to compare against it later and the battery will have moved on by then.

Anything unexpected here, bring the raw bytes back to chat before building.
